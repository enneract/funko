...
void CG_EntityEvent( centity_t *cent, vec3_t position )
{
  ...
  switch( event )
  {
    ...
    //ROTAXfun
    case EV_HUMAN_POISONED:
      DEBUGNAME( "EV_HUMAN_POISONED" );
      {
        particleSystem_t  *ps;
  
        ps = CG_SpawnNewParticleSystem( cgs.media.humanPoisonedPS );
...
/*
    case EV_ALIEN_BUSH_EXPLOSION://ROTAXfun
      DEBUGNAME( "EV_ALIEN_BUSH_EXPLOSION" );
      ByteToDir( es->eventParm, dir );
      CG_AlienBushExplosion( position, dir );
      break;*/
    ...
    //ROTAXfun
    case EV_REPAIRBEAM:
      DEBUGNAME( "EV_REPAIRBEAM" );
      cent->currentState.weapon = WP_LAS_GUN;//TODO
      {
        centity_t *source = &cg_entities[ es->generic1 ];
        centity_t *target = &cg_entities[ es->clientNum ];
        vec3_t    sourceOffset = { 0.0f, 20.0f, 28.0f };
        //vec3_t    targetOffset = { -10.0f, 5.0f, 18.0f };

        if( !CG_IsTrailSystemValid( &source->muzzleTS ) )
        {
          source->muzzleTS = CG_SpawnNewTrailSystem( cgs.media.repairBeamTS );
          ...
          if( CG_IsTrailSystemValid( &source->muzzleTS ) )
          {
            ...
            source->muzzleTSDeathTime = cg.time + 900;
          }
        }
      }
      break;
