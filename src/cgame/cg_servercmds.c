...
static void CG_ServerCommand( void )
{
  ...
  char        prefixName[ 256]; //protoze prefix ale muze obsahovat i skipnotify a area name
  ...
  if( !strcmp( cmd, "chat" ) )
  {
    if( !cg_teamChatsOnly.integer )
    {
      ...
      //rotaxfun
      if (chatClientNum == -1)//pro kompatibilitu se starymi funkcemi, jen se prida parametr -1
      {
      ...
      {
        Q_strncpyz( prefixName, CG_Argv( 3 ), 256 );//bylo MAX_NAME_LENGTH + 9 aLE PROBLEM S AREAMA a skipnotify
        Q_strncpyz( text, CG_Argv( 4 ), MAX_SAY_TEXT );//rotaxfun argument 4 byl 1, pridal jsem clientnum atd a rozdelil zpravu na prefix s name od textu
        if( Q_stricmpn( prefixName, "[skipnotify]", 12 ) )
          trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
        CG_RemoveChatEscapeChar( text );
        CG_RemoveChatEscapeChar( prefixName );
        CG_Printf( "%s%s\n", prefixName, text );
        
        if (!(cg.predictedPlayerState.stats[ STAT_PTEAM ] == PTE_NONE && chatClientNum == cg.clientNum))//svuj text ze specatoru nechci dat do bubliny
        {
          Q_strncpyz( cgs.clientinfo[ chatClientNum ].chatText, text, MAX_SAY_TEXT );
          cgs.clientinfo[ chatClientNum ].chatDurationTime = -1;//Start
          cgs.clientinfo[ chatClientNum ].chatTeam = PTE_NONE;
        }
      }
    }
  ...
  if( !strcmp( cmd, "tchat" ) )
  {
    //rotaxfun
    int chatClientNum = 0;
    int chatTeamChatTeamForAdmins = PTE_NONE;
   ...
   /* char a[256];
    ...
    char f[256];*/
    chatClientNum = atoi(CG_Argv( 1 ));/*
    Q_strncpyz( a, CG_Argv( 0 ), MAX_SAY_TEXT );
    Q_strncpyz( b, CG_Argv( 1 ), MAX_SAY_TEXT );
    Q_strncpyz( c, CG_Argv( 2 ), MAX_SAY_TEXT );
    Q_strncpyz( d, CG_Argv( 3 ), MAX_SAY_TEXT );
    Q_strncpyz( e, CG_Argv( 4 ), MAX_SAY_TEXT );
    Q_strncpyz( f, CG_Argv( 5 ), MAX_SAY_TEXT );
    
    CG_Printf( "%s_%s_%s_%s\n", CG_Argv( 1 ),CG_Argv( 2 ),CG_Argv( 3 ),CG_Argv( 4 ) );*/
    
    if (chatClientNum == -1)//hlaseni egg down
    {
      Q_strncpyz( text, CG_Argv( 2 ), MAX_SAY_TEXT );
      if( Q_stricmpn( text, "[skipnotify]", 12 ) )
      {
    ...
    {
      chatTeamChatTeamForAdmins = atoi(CG_Argv( 2 ));
  
      Q_strncpyz( prefixName, CG_Argv( 3 ), 256 );//bylo MAX_NAME_LENGTH + 9 aLE PROBLEM S AREAMA a skipnotify
      Q_strncpyz( text, CG_Argv( 4 ), MAX_SAY_TEXT );
      if( Q_stricmpn( prefixName, "[skipnotify]", 12 ) )
      {
        if( chatTeamChatTeamForAdmins == PTE_ALIENS )
        {
          trap_S_StartLocalSound( cgs.media.alienTalkSound, CHAN_LOCAL_SOUND );
          if (chatClientNum != -2)//pokud nespecuju, tak bublinim
          {
            Q_strncpyz( cgs.clientinfo[ chatClientNum ].chatText, text, MAX_SAY_TEXT );
            cgs.clientinfo[ chatClientNum ].chatDurationTime = -1;//Start
            cgs.clientinfo[ chatClientNum ].chatTeam = PTE_ALIENS;
          }
        }
        else if( chatTeamChatTeamForAdmins == PTE_HUMANS )
        {
          trap_S_StartLocalSound( cgs.media.humanTalkSound, CHAN_LOCAL_SOUND );
          if (chatClientNum != -2)//pokud nespecuju, tak bublinim
          {
            Q_strncpyz( cgs.clientinfo[ chatClientNum ].chatText, text, MAX_SAY_TEXT );
            cgs.clientinfo[ chatClientNum ].chatDurationTime = -1;//Start
            cgs.clientinfo[ chatClientNum ].chatTeam = PTE_HUMANS;
          }
        }
        else
        {
          trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );//muzu ted zmenit na spravnej zvuk kdyz admin specuje a pise se tymove chatTeamChatTeamForAdmins
          //aby admini videli tymovy text i ze specu
          Q_strncpyz( cgs.clientinfo[ chatClientNum ].chatText, text, MAX_SAY_TEXT );
          cgs.clientinfo[ chatClientNum ].chatDurationTime = -1;//Start
          cgs.clientinfo[ chatClientNum ].chatTeam = chatTeamChatTeamForAdmins;
        }
      }
      CG_RemoveChatEscapeChar( text );
      CG_RemoveChatEscapeChar( prefixName );
