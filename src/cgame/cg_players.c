...
static void CG_PlayerSprites( centity_t *cent )
{
  ...
  if( cent->currentState.eFlags & EF_TALK )
  {
    ...
    CG_PlayerFloatSprite( cent, cgs.media.balloonShader );//ROTAX
    return;
  }
}
...
static void CG_ChatBubblleDisplay( centity_t *cent )
{
  ...
  origin[ 2 ] += BG_FindSpriteHeightForClass( ( cent->currentState.powerups >> 8 ) & 0xFF );
...
*/
  ...
  if( CG_WorldToScreen( origin, &x, &y ) )
  {
    ...
    subY = picY + ( picH * 1.5f ) - ( subH * 1.5f );
      ...
      if( cgs.clientinfo[ cent->currentState.number ].chatTeam == PTE_HUMANS )
        CG_DrawPic( picX, picY, picW, picH, cgs.media.chatBubblle_h );
      else if( cgs.clientinfo[ cent->currentState.number ].chatTeam == PTE_ALIENS )
...
//      trap_R_SetColor( NULL );
 ...
 //       nX -= subH * 1.5f;
     ...
     //CG_DrawField( nX, subY, 4, subH, subH, healthPoints );
      ...
      textW = CG_Text_Width( cgs.clientinfo[ es->clientNum ].chatText, scale, limit );//meri stretched sirku, takze priblizeny je vetsi
      nX = nX + ((picW / 2) - (textW / 2));
    
      CG_Text_Paint( nX, subY, scale, color, cgs.clientinfo[ es->clientNum ].chatText, 0, limit, ITEM_TEXTSTYLE_SHADOWED );
