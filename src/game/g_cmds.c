/*
===========================================================================
Copyright (C) 1999-2005 Id Software, Inc.F
...
//rotax
char marking_h[ 9 ] = "^4[H] ^7";
char marking_a[ 9 ] = "^1[A] ^7";
char marking_s[ 9 ] = "^3[S] ^7";
char marking_temp[100];

//rotax
char * marking(gentity_t *ent)
{
...
//rotax
char * marking_vote(gentity_t *ent)
{ 
  Com_sprintf(marking_temp,
      sizeof(marking_temp), "%s%s^7 asks:", marking(ent), ent->client->pers.netname);
...
void Cmd_Team_f( gentity_t *ent )
{
  ...
  //ROTAXfun
  trap_Argv( 2, safe, sizeof( safe ) );
  if( !Q_stricmp( safe, "safe" ) )
    force = 0;
  ...
  if( !Q_stricmp( s, "spectate" ) )
    team = PTE_NONE;
  else if( level.demoState == DS_PLAYBACK )//rotax
  {
  ...
  G_ChangeTeam( ent, team );
   ...
   if( team == PTE_ALIENS ) {
     if ( oldteam == PTE_HUMANS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 abandoned ^4humans^7 and joined the ^1aliens^7.", ent->client->pers.netname );//rotax
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 joined the ^1aliens^7.", ent->client->pers.netname );//rotax
   }
   else if( team == PTE_HUMANS ) {
     if ( oldteam == PTE_ALIENS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 abandoned the ^1aliens^7 and joined the ^4humans^7.", ent->client->pers.netname );//rotax
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 joined the ^4humans^7.", ent->client->pers.netname );//rotax
   }
   else if( team == PTE_NONE ) {
     if ( oldteam == PTE_HUMANS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 left the ^4humans^7.", ent->client->pers.netname );//rotax
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 left the ^1aliens^7.", ent->client->pers.netname );//rotax
   }
   trap_SendServerCommand( -1, va( "print \"%s\n\"", buf ) );
   G_LogOnlyPrintf("ClientTeam: %s\n",buf);
   
   //ROTAXfun
   ent->client->pers.creditsRewardTime = level.time;
}
...
//    name, Q_COLOR_ESCAPE, color, message ) );
  ...
  if (ent->client->pers.teamSelection == PTE_NONE)//pokud specuju, nezobrazim tento text v bublinach
    clientNum = -2;
  else
    clientNum = ent->client->ps.clientNum; 

  trap_SendServerCommand( other-g_entities, va( "%s %i %i \"%s%s%s\"\"%c%c%s\"",
    ( mode == SAY_TEAM || mode == SAY_ACTION_T ) ? "tchat" : "chat",
    clientNum,
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText )
{
  ...
  if (g_chatTeamPrefix.integer)
    Com_sprintf( prefix, sizeof( prefix ), marking(ent));
  else
  ...
  switch( mode )
  {
    ...
    case SAY_ALL:
      ...
      Com_sprintf( name, sizeof( name ), "%s%s%c%c: ", prefix,
                   ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo "%s%s%c%c"EC"
      color = COLOR_GREEN;
      G_DemoCommand( DC_SERVER_COMMAND, va( "chat \"%s^2%s\"", name, chatText ) );//rotax
      break;
    ...
    case SAY_TEAM:
      ...
      if( Team_GetLocationMsg( ent, location, sizeof( location ) ) )
        Com_sprintf( name, sizeof( name ), "(%s%c%c) (%s): ",
          ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, location );//rotaxfun bylo EC"(%s%c%c"EC") (%s)"EC"
      else
        Com_sprintf( name, sizeof( name ), "(%s%c%c): ",
          ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo EC"(%s%c%c"EC")"EC": "
      
      if( ent->client->pers.teamSelection == PTE_NONE )
        color = COLOR_YELLOW;
      else
        color = COLOR_CYAN;
      G_DemoCommand( DC_SERVER_COMMAND, va( "tchat \"%s^%c%s\"", name, color, chatText ) );//rotax
      break;
    ...
    case SAY_ACTION:
      ...
      Com_sprintf( name, sizeof( name ), "^2%s^7%s%s%c%c ", g_actionPrefix.string, prefix,
                   ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo "^2%s^7%s%s%c%c"EC" "
      color = COLOR_WHITE;
      G_DemoCommand( DC_SERVER_COMMAND, va( "chat \"%s%s\"", name, chatText ) );//rotax
      break;

    case SAY_ACTION_T:
      G_LogPrintf( "actionteam: %s%s^7: %s^7\n", prefix, ent->client->pers.netname, chatText );
      if( Team_GetLocationMsg( ent, location, sizeof( location ) ) )
        Com_sprintf( name, sizeof( name ), "^5%s^7%s%c%c(%s) ", g_actionPrefix.string, 
          ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, location );//rotaxfun bylo EC"^5%s^7%s%c%c"EC"(%s)"EC" "
      else
        Com_sprintf( name, sizeof( name ), "^5%s^7%s%c%c ", g_actionPrefix.string, "^5%s^7%s%c%c ",
          ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo EC"^5%s^7%s%c%c"EC""EC" "
      color = COLOR_WHITE;
      G_DemoCommand( DC_SERVER_COMMAND, va( "tchat \"%s%s\"", name, chatText ) );//rotax
      break;
      ...
      case SAY_ADMINS:
        if( G_admin_permission( ent, ADMF_ADMINCHAT ) ) //Differentiate between inter-admin chatter and user-admin alerts
        {
         ...
         Com_sprintf( name, sizeof( name ), "%s[ADMIN]%s%c%c: ", prefix,
                    ( ent ) ? ent->client->pers.netname : "console", Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo "%s[ADMIN]%s%c%c"EC": "
         color = COLOR_MAGENTA;
        }
        else
        {
          G_LogPrintf( "say_admins: [PLAYER]%s^7: %s^7\n", ent->client->pers.netname, chatText );
          Com_sprintf( name, sizeof( name ), "%s[PLAYER]%s%c%c: ", prefix,
            ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );//rotaxfun bylo "%s[PLAYER]%s%c%c"EC": "
          color = COLOR_MAGENTA;
        }
        break;
  }
...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  else if( !Q_stricmp( arg1, "nextmap" ) )
  {
    ...
    Com_sprintf( level.voteDisplayString,
        sizeof( level.voteDisplayString ), "Set the ^1next^7 map to '%s^7'", arg2 );//ROTAX
    level.votePassThreshold = g_mapVotesPercent.integer;
  }
  else if( !Q_stricmp( arg1, "draw" ) )
  {
   ...
   else if( !Q_stricmp( arg1, "poll" ) )
    {
      ...
      Com_sprintf( level.voteDisplayString,
          sizeof( level.voteDisplayString ), "[Poll] \'%s^7'", arg2plus );//ROTAX added ^7
   }
   else if( !Q_stricmp( arg1, "sudden_death" ) ||
     !Q_stricmp( arg1, "suddendeath" ) ||
     !Q_stricmp( arg1, "sd" ))//rotax pridano sd
   {
   ...
   else if( !Q_stricmp( arg1, "armageddon" ) ||
     !Q_stricmp( arg1, "arma" ))//ROTAX
   {
     if(!g_rotaxARMAGEDDON.integer)
     {
       trap_SendServerCommand( ent-g_entities, "print \"callvote: Armageddon is disabled.\n\"" );
       return;
     } 	  
     else if(armageddon == 1)
     {
       trap_SendServerCommand( ent-g_entities, "print \"callvote: Armageddon is right NOW!\n\"" );
       return;
     }
     else if (( level.time - level.startTime ) < 1200000 && !G_admin_permission( ent, ADMF_CAN_PERM_BAN ) )
     {
       trap_SendServerCommand( ent-g_entities, "print \"callvote: Armageddon can be called 20 minutes after start.\n\"" );
       return;
     }
     else 
     {
      //projdu vsechny entity
      int je_co_nicit = 0;
      arma_ent = &g_entities[ 0 ];

      for ( i = 1, arma_ent = g_entities + i; i < level.num_entities; i++, arma_ent++ )
      {
        if( arma_ent->s.eType != ET_BUILDABLE )
          continue;

        //pokud zije alespon jedna budova, krome spawnu atd., tak ok
        if( ((arma_ent->s.modelindex != BA_A_OVERMIND && arma_ent->s.modelindex != BA_H_ARMOURY && arma_ent->s.modelindex != BA_H_MEDISTAT && arma_ent->s.modelindex != BA_H_REACTOR && arma_ent->s.modelindex != BA_H_REPEATER)) && arma_ent->health > 0 )
        {
          je_co_nicit = 1;
          break;
        }
      ...
      if (je_co_nicit == 1)
      {
        Com_sprintf( level.voteString, sizeof( level.voteString ), "armageddon" );
        Com_sprintf( level.voteDisplayString,
            sizeof( level.voteDisplayString ), "Summon armageddon?" );
        level.votePassThreshold = g_mapVotesPercent.integer;
      }
      else
      {
        trap_SendServerCommand( ent-g_entities, "print \"callvote: Everything is already destroyed...\n\"" );
        return;
      }
     }
   }
   else if( !Q_stricmp( arg1, "delay" ) )//ROTAXfun
   {
     ...
     {
       level.votePassThreshold = g_suddenDeathVotePercent.integer;
       Com_sprintf( level.voteString, sizeof( level.voteString ), "delay" );
       Com_sprintf( level.voteDisplayString,
           sizeof( level.voteDisplayString ), "Delay Sudden Death by 10 minutes" );
     }
   }
  else
  {
    ...
    trap_SendServerCommand( ent-g_entities, "print \"Valid vote commands are: "
      "map, map_restart, nextmap, kick, mute, unmute, poll, sudden_death (sd), armageddon (arma), delay.\n" );//rotax
    return;
  }
  ...
  else if(!Q_stricmp( level.voteString, "armageddon" ))//ROTAX
  {
    Q_strcat( level.voteDisplayString, sizeof( level.voteDisplayString ), " (Needs > 70 percent)" );
  }
  ...
  Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ),
      va ("%s %s", marking_vote(ent), level.voteDisplayString ) );

  trap_SendServerCommand( -1, va( "print \"^2____VOTE____^7 %s\n\"", level.voteDisplayString ) );
...
//ROTAX
int G_FindConfigstringIndexROTAX( char *name, int start, int max )
{
  int   i;
  char  s[ MAX_STRING_CHARS ];
  char  wtf[1024];
  
  Q_strncpyz( wtf, name, sizeof( wtf ) );
  ...
  for( i = 1; i < max; i++ )
  {
    ...
    if( !s[ 0 ] )
      ...
      //trap_SendServerCommand( -1, va("print \"((%s))\n\"", name) );
      //Why the hell variable "name" not work??? Try uncomment line above and in game type "/playsound something".
      //It will print one correct line and next lines are only mess (should be same). Don't know why. I was spend 4 hours with this
      //and then I resolved it trough "wtf" variable.
...
//ROTAX
//taunt
//seznam dostupnych zvuku
void Cmd_Playsound_f( gentity_t *ent )
{
  char msg[ 64 ];
  char map[ MAX_STRING_CHARS ];//rotax
  ...
  if( g_floodMinTime.integer )
    if ( G_Flood_Limited( ent ) )
    {
      trap_SendServerCommand( ent-g_entities, "print \"Playsound is flood-limited; wait before talking again\n\"" );
      return;
    }
  ...
  index = G_FindConfigstringIndexROTAX( va("playsound/%s/%s.wav", map, msg), CS_SOUNDS, MAX_SOUNDS );
  ...
  {    
    ...
    G_LogPrintf( "playsound: %s^7: %s^7\n", ent->client->pers.netname, msg );/*
    Com_sprintf( name2, sizeof( name2 ), "^1(PS)^7 %s%s%c%c"EC": ", prefix,
                 ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );
    color = COLOR_GREEN;*/

    //Com_sprintf( text, sizeof( text ), "%s^7", msg );
    ...
    {
      other = &g_entities[ j ];
      G_SayTo( ent, other, SAY_ALL, color, name2, msg, prefix );
    }*/
    
    //BG_AddPredictableEventToPlayerstate( EV_PAIN, G_SoundIndex( va("playsound/%s/%s.wav", map, msg) ), &ent->client->ps );
    //G_AddPredictableEvent( ent, EV_PAIN, index );
    //G_AddPredictableEvent( ent, EV_GENERAL_SOUND, index );
    //G_AddPredictableEvent( ent, EV_GLOBAL_SOUND, G_SoundIndex( va("playsound/%s/%s.wav", map, msg) ) );
    G_AddEvent( ent, EV_GENERAL_SOUND, index );
  }
  else
    trap_SendServerCommand( ent-g_entities, "print \"Playsound: requested sound not found\n\"" );
}
...
void Cmd_Vote_f( gentity_t *ent )
{
  ...
  if( msg[ 0 ] == 'y' || msg[ 1 ] == 'Y' || msg[ 1 ] == '1' )
  {
    ...
    //ROTAXfun
    trap_SendServerCommand( -1, va("print \"^7Vote: %s%s ^7voted ^2YES^7\n\"", marking(ent), ent->client->pers.netname ) );
  ...
  else
  {
    ...
    //ROTAXfun
    trap_SendServerCommand( -1, va("print \"^7Vote: %s%s ^7voted ^1NO^7\n\"", marking(ent), ent->client->pers.netname ) );
...
void Cmd_CallTeamVote_f( gentity_t *ent )
{
  ...
  for( i = 0 ; i < level.maxclients ; i++ )
  {
    ...
    if( level.clients[ i ].ps.stats[ STAT_PTEAM ] == team )
    {
      ...
      trap_SendServerCommand( i, va("print \"" S_COLOR_WHITE
            "^2____TEAMVOTE____^7 %s %s^7 \n\"", marking_vote(ent), level.teamVoteDisplayString[ cs_offset ] ) );
    }
             ...
             level.clients[ i ].pers.teamSelection == PTE_NONE ) )
    {
      ...
      //      "called a team vote: %s^7 \n\"", ent->client->pers.netname, level.teamVoteDisplayString[ cs_offset ] ) );
      trap_SendServerCommand( i, va("print \"" S_COLOR_WHITE
            "^2____TEAMVOTE____^7 ^6[Admins]^7 %s %s^7 \n\"", marking_vote(ent), level.teamVoteDisplayString[ cs_offset ] ) );
    }
  }
...
void Cmd_TeamVote_f( gentity_t *ent )
{
  ...
  if( msg[ 0 ] == 'y' || msg[ 1 ] == 'Y' || msg[ 1 ] == '1' )
  {
    ...
    //ROTAXfun
    G_LogPrintf("Vote: %s%s ^5voted YES\n", marking(ent), ent->client->pers.netname );
    G_TeamCommand( ent->client->pers.teamSelection, va( "tchat -1 \"^5Vote: %s%s ^5voted ^2YES^5\"", marking(ent), ent->client->pers.netname ) );
  }
  else
  {
    ...
    //ROTAXfun
    G_LogPrintf("Vote: %s%s ^5voted NO\n", marking(ent), ent->client->pers.netname );
...
*/
      if(g_suddenDeath.integer)//rotax
      {
        /* - vraceno
        if (traceEnt->s.modelindex == BA_H_REACTOR)
        {
          ...
          int i = 0;
          gentity_t *check_ent;
          check_ent = &g_entities[ 0 ];
          for ( i = 1, check_ent = g_entities + i; i < level.num_entities; i++, check_ent++ )
          {
            if( check_ent->s.eType != ET_BUILDABLE )
              continue;

            if( check_ent->s.modelindex == BA_H_REACTOR && check_ent->health > 0 )
            {
              pocet_rc++;
            }
          }
        ...
        else if (traceEnt->s.modelindex == BA_A_OVERMIND)
        {
          //pocet om
          int pocet_om = 0;
          int i = 0;
          ...
          for ( i = 1, check_ent = g_entities + i; i < level.num_entities; i++, check_ent++ )
          {
            ...
            if( check_ent->s.modelindex == BA_A_OVERMIND && check_ent->health > 0 )
            {
              pocet_om++;
            }
          }
        ...
        {
          trap_SendServerCommand( ent-g_entities,
            "print \"You can't deconstruct buildings in sudden death\n\"" );
          return;
        }
      }
    
      if(g_friendlyFire.integer == 0 || g_friendlyBuildableFire.integer == 0)//rotax pokud je ffboff, neme deconovat
      {
        trap_SendServerCommand( ent-g_entities,
          "print \"Deconstructing is off.\n\"" );
        return;
      }

      //rotax pokud nema jmeno, nemuze deconovat
...
void Cmd_ActivateItem_f( gentity_t *ent )
{
  ...
  {
    //ROTAX
    if ( !(ent->client->ps.weapon == WP_LUCIFER_CANNON && ent->client->ps.stats[ STAT_MISC ] > 0) )
...
void Cmd_Build_f( gentity_t *ent )
{
        ...
        ( team == PTE_HUMANS && BG_FindStagesForBuildable( buildable, g_humanStage.integer ) ) ) )
  {
    ...
    switch( G_CanBuild( ent, buildable, dist, origin ) )
    {
      ...
      /*ROTAXfun !TADY CHYB9 TEN HOVEL, proto to nic nehlasi kdyz chci v sd stavet hovel (normalne) - vraceno
...
qboolean G_FollowNewClient( gentity_t *ent, int dir )
{
  ...
  do
  {
    ...
    if( level.clients[ clientnum ].pers.connected != CON_CONNECTED &&
        !level.clients[ clientnum ].pers.demoClient )//rotax
      continue;
...
void Cmd_PTRCVerify_f( gentity_t *ent )
{
  ...
  //rotax - zakaz automatickeho pripojeni do teamu po odpojeni/kicku kdyz je nektery team zamknut.
  //if( connection && connection->clientNum == -1 )
  if( (connection && connection->clientNum == -1) && (!level.humanTeamLocked && !level.alienTeamLocked) )
  {
    // valid code
...
commands_t cmds[ ] = {
  ...
  //ROTAX
  { "playsound", CMD_TEAM|CMD_LIVING, Cmd_Playsound_f }
};
...
void G_PrivateMessage( gentity_t *ent )
{
  ...
  for( i=0; i < matches; i++ )
  {
    ...
    trap_SendServerCommand( pids[ i ], va(
      "chat -1 \"%s^%c -> ^7%s^7: (%d recipients): ^%c%s^7\" %i",//rotaxfun -1
