...
void ClientImpacts( gentity_t *ent, pmove_t *pm )
{
  ...
  for( i = 0; i < pm->numtouch; i++ )
  {
    ...
    //ROTAX if( ent->client && other->client )
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd )
{
  ...
  if ( client->sess.spectatorState == SPECTATOR_FOLLOW )
  {
    ...
    if ( client->sess.spectatorClient >= 0 )
    {
      ...
      if ( cl->sess.sessionTeam != TEAM_SPECTATOR || cl->pers.demoClient )//rotax
        doPmove = qfalse;
    }
  }
...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  while( client->time1000 >= 1000 )
  {
    ...
    if( client->ps.stats[ STAT_STATE ] & SS_POISONCLOUDED )
    {
      ...
      //ROTAXfun
      {
        gentity_t *human_poisoned;
        human_poisoned = G_TempEntity( client->ps.origin, EV_HUMAN_POISONED );
        human_poisoned->s.clientNum = ent->s.clientNum;
      }
    }
...
#define USE_OBJECT_RANGE 64
  ...
  //ROTAXfun if( ent->client->lastKillTime + FREEKILL_PERIOD < level.time )
  if( ent->client->pers.creditsRewardTime + FREEKILL_PERIOD < level.time )
  {   
    //ROTAXfun if( !g_suddenDeath.integer ) {
      if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
        G_AddCreditToClient( ent->client, FREEKILL_ALIEN, qtrue );
      if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
        G_AddCreditToClient( ent->client, FREEKILL_HUMAN, qtrue );
    //ROTAXfun }
    //ROTAXfun ent->client->lastKillTime = level.time;
...
void SpectatorClientEndFrame( gentity_t *ent )
{
  ...
  if( ent->client->sess.spectatorState == SPECTATOR_FOLLOW )
  {
    ...
    if( clientNum >= 0 )
    {
      ...
      if( cl->pers.connected == CON_CONNECTED || cl->pers.demoClient )//rotax
      {
