...
static void PM_CheckLadder( void )
{
  ...
  //if( ( trace.fraction < 1.0f ) && (( trace.surfaceFlags & SURF_LADDER ) || trace.entityNum == BA_H_REACTOR) )
    pml.ladder = qtrue;
  else
    pml.ladder = qfalse;
}
...
static void PM_GroundClimbTrace( void )
{
  ...
  for( i = 0; i <= 4; i++ )
  {
    ...
    */
    ...
    //ROTAXfun chozeni i po hracich
    if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && ( trace.entityNum >= MAX_CLIENTS && i != 4) )
    //if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && !( trace.entityNum >= MAX_GENTITIES && i != 4 ) )
    {
      if( i == 2 || i == 3 )
      {
        if( i == 2 )
...
static void PM_Weapon( void )
{
      ...
      if( !( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) )
      {
        if( pm->cmd.weapon <= 32 )
        {
          ...
          if (pm->ps->weapon == WP_LUCIFER_CANNON && pm->ps->stats[ STAT_MISC ] > 0 )
            pm->cmd.weapon = WP_LUCIFER_CANNON;
  ...
  if( attack3 )
  {
    if( BG_WeaponHasThirdMode( pm->ps->weapon ) )
    {
      ...
      if( (pm->ps->weapon == WP_ALEVEL1 || pm->ps->weapon == WP_ALEVEL1_UPG) && !ammo )
      {
        PM_AddEvent( EV_NOAMMO );
        pm->ps->weaponTime += 200;
        return;
