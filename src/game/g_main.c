...
int  automapchange = 0;//rotax

#define QVM_NAME       "Lakitu7's QVM"
#define QVM_VERSIONNUM      "5.5"

//ROTAXfun
#define MAX_IP_TO_COUNTRY_MEMORY (15*1024*1024) //15 mb
static char ip_to_country_memory[MAX_IP_TO_COUNTRY_MEMORY];
IpToCountryList *iptoclist = NULL;		//Ip2Country database
...
vmCvar_t  g_rotaxHP;//rotax
vmCvar_t  g_rotaxARMAGEDDON;//ROTAX
vmCvar_t  g_iptocountryFile;//ROTAXfun
vmCvar_t  g_rotaxJumppadPower;//ROTAXfun

int  armageddon = 0;//ROTAX
int  armageddon_time = 0;//ROTAX
int  armageddon_struct_count_a = 0;//ROTAX
int  armageddon_struct_count_h = 0;//ROTAX
int  armageddon_kill_count_a = 0;//ROTAX
int  armageddon_countdown = 10;//ROTAX
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { NULL, "P", "", CVAR_ROM, 0, qfalse  },//rotax
  { NULL, "ff", "0", CVAR_ROM, 0, qfalse  },//rotax
  { NULL, "qvm_version", QVM_NAME " " QVM_VERSIONNUM " (" __DATE__ ", " __TIME__ ")", CVAR_ROM, 0, qfalse  },//rotax
  ...
  // change anytime vars
  { &g_timelimit, "timelimit", "45", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//rotax
  { &g_suddenDeathTime, "g_suddenDeathTime", "30", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//rotax
  { &g_suddenDeathMode, "g_suddenDeathMode", "2", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//rotax
  { &g_suddenDeath, "g_suddenDeath", "0", CVAR_NORESTART, 0, qtrue },//rotax

  { &g_synchronousClients, "g_synchronousClients", "0", CVAR_SYSTEMINFO, 0, qfalse  },

  { &g_friendlyFire, "g_friendlyFire", "0", CVAR_ARCHIVE, 0, qtrue  },//rotax
  ...
  { &g_friendlyBuildableFire, "g_friendlyBuildableFire", "0", CVAR_ARCHIVE, 0, qtrue  },//rotax
  ...
  { &g_humanBuildPoints, "g_humanBuildPoints", DEFAULT_HUMAN_BUILDPOINTS, 0, 0, qfalse  },//rotax
  { &g_alienBuildPoints, "g_alienBuildPoints", DEFAULT_ALIEN_BUILDPOINTS, 0, 0, qfalse  },//rotax
  ...
  { &g_allowShare, "g_allowShare", "0", CVAR_ARCHIVE, 0, qfalse},//rotax
  { &g_banNotice, "g_banNotice", "", CVAR_ARCHIVE, 0, qfalse  },

  { &g_rotaxHP, "g_rotaxHP", "0", CVAR_ARCHIVE, 0, qfalse  },//ROTAX
  { &g_rotaxARMAGEDDON, "g_rotaxARMAGEDDON", "0", CVAR_ARCHIVE, 0, qfalse  },//ROTAX
  { &g_iptocountryFile, "g_iptocountryFile", "ip-to-country.bin", CVAR_ARCHIVE, 0, qfalse  },//ROTAXfun
...
void G_DemoSetClient( void );//rotax
void G_DemoRemoveClient( void );//rotax
void G_DemoSetStage( void );//rotax
...
//ROTAXfun
void G_LoadIp2CountryInfo(const char *filename, IpToCountryList *infoList, int camelCaseNames)
{
    unsigned int byteSize;
    fileHandle_t dataFile;

    trap_FS_FOpenFile( filename, &dataFile, FS_READ );
    if (!dataFile) {
        G_Printf( "ERROR: Could not load ip to country data file!\n" );
        return;
    }
    infoList->size = 0;
    infoList->infoArray = NULL;

    //Read the rownumber
    trap_FS_Read(&infoList->size, sizeof(infoList->size),dataFile);
    G_Printf( "Rows in file: %d\n", infoList->size );
    //Create a new array of ip info
    byteSize = infoList->size*sizeof(IpToCountryInfo);
    infoList->infoArray = (IpToCountryInfo*) ip_to_country_memory;

    //Fill the array with info
    trap_FS_Read(infoList->infoArray, byteSize, dataFile);

    trap_FS_FCloseFile(dataFile);
}
...
{
  int i;
  char buffer[ MAX_CVAR_VALUE_STRING ];//rotax
  int a, b;//rotax
  ...
  trap_Cvar_VariableStringBuffer( "session", buffer, sizeof( buffer ) );//rotax
  sscanf( buffer, "%i %i", &a, &b );//rotax
  if ( a != trap_Cvar_VariableIntegerValue( "sv_maxclients" ) ||
       b != trap_Cvar_VariableIntegerValue( "sv_democlients" ) )//rotax
  ...
  //ROTAXfun hack (delay vote affect this, server.cfg not affect this now)
  trap_Cvar_Set( "g_suddenDeathTime", "30" );
  
  //rotax nastaveni ff zpatky na default
  trap_Cvar_Set( "g_friendlyFire", "1" );
  trap_Cvar_Set( "g_friendlyBuildableFire", "1" );
  trap_Cvar_Set( "g_friendlyFireHumans", "1" );
  ...
  if (iptoclist == NULL) {
      G_Printf ("Loading IpToCountry database... \n");
      iptoclist = (IpToCountryList*) G_Alloc(sizeof(IpToCountryList));
      iptoclist->infoArray = NULL;
      iptoclist->size = 0;

      G_LoadIp2CountryInfo(g_iptocountryFile.string,iptoclist,qtrue);
      G_Printf ("loaded %d rows\n",iptoclist->size);
  }
...
void G_ShutdownGame( int restart )
{
  ...
  // clear all demo clients//rotax
  clients = trap_Cvar_VariableIntegerValue( "sv_democlients" );//rotax
  for( i = 0; i < clients; i++ )//rotax
    trap_SetConfigstring( CS_PLAYERS + i, NULL );//rotax
}
...
void G_CalculateStages( void )
{
  ...
  static int    lastAlienStage  = -1;//rotax
  static int    lastHumanStage  = -1;//rotax
  ...
  if ( level.demoState == DS_RECORDING &&
       ( trap_Cvar_VariableIntegerValue( "g_alienStage" ) != lastAlienStage ||
         trap_Cvar_VariableIntegerValue( "g_humanStage" ) != lastHumanStage ) )//rotax
  {
...
void CalculateRanks( void )
{
  ...
  for( i = 0; i < level.maxclients; i++ )
  {
    ...
    if ( level.clients[ i ].pers.connected != CON_DISCONNECTED ||
         level.clients[ i ].pers.demoClient )//rotax
    {
...
void ExitLevel( void )
{
  ...
  else if (Q_stricmp( map, "atcs" ) != 0)//ROTAX
    trap_SendConsoleCommand( EXEC_APPEND, va( "map atcs\n" ) );
  else
...
void CheckExitRules( void )
{
        ...
        ( level.numLiveAlienClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^4---===>>> Humans win <<<===---^7\n\"");
             ...
             ( level.numLiveHumanClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^1---===>>> Aliens win <<<===---^7\n\"");
...
void CheckVote( void )
{
  ...
  {
    //ROTAX
    if( !Q_stricmp( level.voteString, "armageddon" ) )
    {
      if( voteYesPercent > 70 )
      {
    ...
    else if( voteYesPercent> votePassThreshold || level.voteNo == 0 )
    {
      // execute the command, then remove the vote
      trap_SendServerCommand( -1, va("print \"Vote passed (%d - %d)\n\"", 
            level.voteYes, level.voteNo ) );
...
void G_RunFrame( int levelTime )
{
  ...
  for( i = 0; i < ( level.demoState == DS_PLAYBACK ? g_maxclients.integer : level.num_entities ); i++, ent++ )//rotax
  {
    if( !ent->inuse )
      continue;
  ...
  //rotax automaticke spusteni atcs kdyz neni nikdo pripojen
  if (level.numConnectedClients == 0 && automapchange == 0)
  {
    trap_Cvar_VariableStringBuffer( "mapname", map, sizeof( map ) );
    if (Q_stricmp( map, "atcs" ) != 0)
    {
      automapchange = level.time + 15000;
    }
  }
  if (automapchange > 0 && automapchange <= level.time)
  {
    trap_SendConsoleCommand( EXEC_APPEND, va( "map atcs\n" ) );
    level.restarted = qtrue;
    automapchange = 0;
    G_LogPrintf( "Automatically set to atcs map due no players\n");
  }
  if (automapchange > 0 && level.numConnectedClients != 0)
  {
    automapchange = 0;
  }
  ...
  {
    //odpocet
    if (armageddon_countdown >= 0 && armageddon_time < level.time)
    {
      armageddon_time = level.time + 1000; 
      
      if( armageddon_countdown > 0 )
        trap_SendServerCommand( -1, va( "cp \"^1Armageddon^7\nT - %i\"", armageddon_countdown ) );
      else if( armageddon_countdown == 0 )
      {
        trap_SendServerCommand( -1, "print \"^3-->^7 Armageddon was ^1started!\n\"" );
        trap_SendServerCommand( -1, "cp \"Armageddon was ^1started!\"" );
        armageddon_time = 0;
      }

      armageddon_countdown--;
    }
    
    //rychlost niceni
    if ( armageddon_time < level.time && armageddon_countdown == -1)
    {
      int armageddon_konec = 1;
      int armageddon_human = 0;
      int armageddon_alien = 0;
      int armageddon_all_aliens_killed = 1;
      armageddon_time = level.time + 250; 
      arma_ent = &g_entities[ 0 ];
      arma_players = &g_entities[ 0 ];
      ...
      for ( i = 1, arma_ent = g_entities + i; i < level.num_entities; i++, arma_ent++ )
      {
        ...
        //zabiju silne alieny
        /*
        if (arma_ent->s.eType == ET_PLAYER && arma_ent->health > 0 && arma_ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS && (arma_ent->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_LEVEL3_UPG || arma_ent->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_LEVEL4))
        {
          G_Damage( arma_ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_UNKNOWN );
          armageddon_all_aliens_killed = 0;
          armageddon_kill_count_a++;
          continue;
        }
        */
    
        //pokud to neni spawn atd., zabiju
        ...
        {
          //zabiju jednu vec u humanu a jednu u alienu najednou
          if (armageddon_human == 0 && arma_ent->s.eType == ET_BUILDABLE
            && (arma_ent->s.modelindex == BA_H_MGTURRET
                || arma_ent->s.modelindex == BA_H_TESLAGEN
                || arma_ent->s.modelindex == BA_H_DCC
                || arma_ent->s.modelindex == BA_H_SPAWN))
          {
            G_Damage( arma_ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
            armageddon_human = 1;
            armageddon_struct_count_h++;
            armageddon_konec = 0;
            continue;
          }
          else if (armageddon_alien == 0 && arma_ent->s.eType == ET_BUILDABLE
            && (arma_ent->s.modelindex == BA_A_BARRICADE
                || arma_ent->s.modelindex == BA_A_ACIDTUBE
                || arma_ent->s.modelindex == BA_A_TRAPPER
                || arma_ent->s.modelindex == BA_A_BOOSTER
                || arma_ent->s.modelindex == BA_A_HIVE
                || arma_ent->s.modelindex == BA_A_HOVEL
                || arma_ent->s.modelindex == BA_A_SPAWN))
          {
            G_Damage( arma_ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
            armageddon_alien = 1;
            armageddon_struct_count_a++;
            armageddon_konec = 0;
            continue;
          }
    
          //ukoncim, aby to neprojizdelo zbytecne dalsi a nedelalo mezeru
          if (armageddon_alien == 1 && armageddon_human == 1)
            break;
        }
      }
            
      //konec armageddonu
      if (armageddon_konec == 1 && armageddon_all_aliens_killed == 1)
      {
        //nakonec znicim OM
        /*
        arma_ent = &g_entities[ 0 ];
        for ( i = 1, arma_ent = g_entities + i; i < level.num_entities; i++, arma_ent++ )
        {
          if (arma_ent->s.eType == ET_BUILDABLE && arma_ent->s.modelindex == BA_A_OVERMIND && arma_ent->health > 0)
          {
            G_Damage( arma_ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
            break;
          }
        ...
        for ( i = 1, arma_ent = g_entities + i; i < level.num_entities; i++, arma_ent++ )
        {
          if (arma_ent->s.eType == ET_BUILDABLE && arma_ent->s.modelindex == BA_H_REACTOR && arma_ent->health > 0)
          {
            G_Damage( arma_ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
            break;
          }
        ...
        trap_SendServerCommand( -1, va("print \"^3-->^7 Armageddon was ^2ended. ^7Destroyed ^4H: %i ^7and ^1A: %i ^7structs.\n\"", armageddon_struct_count_h, armageddon_struct_count_a) );
        trap_SendServerCommand( -1, "cp \"Armageddon was ^2ended.\"" );
        armageddon = 0;
        armageddon_time = 0;
        armageddon_struct_count_h = 0;
        armageddon_struct_count_a = 0;
        armageddon_kill_count_a = 0;
        armageddon_countdown = 10;
        armageddon_all_aliens_killed = 0;
      }
    }
  }
