...
#include "../qcommon/IpToCountryResolver.h" //ROTAXfun
...
typedef struct
{
  ...
  qboolean            aimbot;//ROTAX
  IpToCountryInfo     *ip2c;//ROTAXfun
  int                 creditsRewardTime;//ROTAXfun
} clientPersistant_t;
...
typedef enum
{
  ...
  //ROTAXfun
  //IBE_OVERMIND_AFTER_SD, - vraceno
  IBE_HOVEL_AFTER_SD,
  //IBE_RC_AFTER_SD, - vraceno
...
void G_DemoCommand( demoCommand_t cmd, const char *string );//rotax
int  G_TimeTilSuddenDeath( void );
void CheckMsgTimer( void );
qboolean G_Flood_Limited( gentity_t *ent );

void G_LoadIp2CountryInfo(const char *filename, IpToCountryList *infoList, int camelCaseNames);//ROTAXfun
...
extern IpToCountryList *iptoclist;//ROTAXfun
...
extern  vmCvar_t  g_rotaxHP;//rotax
extern  vmCvar_t  g_rotaxARMAGEDDON;//ROTAX

extern int armageddon;//ROTAX
...
void      trap_DemoCommand( demoCommand_t cmd, const char *string );//rotax
