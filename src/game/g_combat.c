...
//**
  ...
  if( !tk )
  {
    ...
    if ( g_rotaxHP.integer == 0 
        ...
        || meansOfDeath == MOD_HSPAWN
        || meansOfDeath == MOD_ASPAWN
        || meansOfDeath == MOD_MGTURRET
        || meansOfDeath == MOD_TESLAGEN
        || meansOfDeath == MOD_ATUBE
        || meansOfDeath == MOD_BUSH
        || meansOfDeath == MOD_OVERMIND
        || meansOfDeath == MOD_REACTOR
        || meansOfDeath == MOD_SLOWBLOB
        || meansOfDeath == MOD_SWARM
    ...
    {
      //rotax zkopirovano z cg_event.c
      //*********************************************
      message2 = "";
      ...
      if( killer != ENTITYNUM_WORLD )
      {
        ...
        trap_SendServerCommand( -1,
          va( "print \"%s^7 %s %s^7%s^7, ^2%i^7 HP left\n\"",
          self->client->pers.netname, message, attacker->client->pers.netname, message2, attacker->client->ps.stats[ STAT_HEALTH ] ) );   
          rbb = 1;
        }
      }
  ...
  if( attacker && attacker->client )
  {
   ...
   if( g_devmapKillerHP.integer && g_cheats.integer ) 
   {
    ...
    else
    {
      ...
      attacker->client->pers.creditsRewardTime = level.time;
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( !( dflags & DAMAGE_NO_PROTECTION ) )
  {
    ...
    {
  		if( !g_friendlyBuildableFire.integer || (g_suddenDeath.integer && (targ->s.modelindex == BA_H_REACTOR || targ->s.modelindex == BA_A_OVERMIND || targ->s.modelindex == BA_A_SPAWN || targ->s.modelindex == BA_H_SPAWN || targ->s.modelindex == BA_H_REPEATER)))//rotax od || vcetne
  			return;
  			
  		if ( level.numAlienSpawns == 1 && targ->s.modelindex == BA_A_SPAWN)//rotax
  		  return;
  
  		if ( level.numHumanSpawns == 1 && targ->s.modelindex == BA_H_SPAWN)//rotax
  		  return;
  ...
  if( take )
  {
    ...
    //ROTAX
    /* pokud je budova postavena, pokud nema max hp, neukazovat ze poskozuje om
    if (targ->s.eType == ET_BUILDABLE && attacker->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0_UPG && ( attacker->client->buttons & BUTTON_USE_HOLDABLE ))
